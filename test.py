...
def create_child_logger(com_port: str, serial_number):
    logger = logging.getLogger(f"child_{com_port}")
    logger.setLevel(logging.DEBUG)

    if not logger.handlers:
        log_file = f"{result_folder}/{serial_number}.log"
...
def print_summary_error_and_exit(message: str, error_code: ExitCodes, logging):
...
def serial_initialization(serial_port: str, com: Communication, logging):
    logging.info(
...
def send_data(com: Communication, data: bytes, type: MessageType) -> bool:
    ...
    if len(hdlc_frame) > com.EMBEDDED_HDLC_BUFFER_MAX_SIZE:
        print_summary_error_and_exit(
            ...
            ExitCodes.HDLC_PACKET_TOO_LARGE, logging
        )
    ...
    try:
        ...
        print_summary_error_and_exit(msg, ExitCodes.SERIAL_ERROR, logging)
...
def rx_callback(com: Communication, logging):
    read = bytes()
    while True:
        read += com.ser.read(1)
        try:
...
def initialize_dut(args: argparse, com: Communication, RX_RSSI_THRESHOLD_DBM, TX_RSSI_THRESHOLD_DBM, logging):
    ...
    try:
        ...
        print_summary_error_and_exit(
            ...
            ExitCodes.RECEPTION_TIMEOUT, logging
        )
    ...
    if test_init_status == TestInitStatus.CONFIG_ERROR:
        print_summary_error_and_exit(
            ...
            ExitCodes.INVALID_PARAMETERS,logging
        )
    elif test_init_status == TestInitStatus.TESTING_ALREADY_DONE:
        print_summary_error_and_exit(
            "Testing has already been performed on this NIC!",
            ExitCodes.TEST_ALREADY_DONE,logging
        )


def perform_external_flash_testing(com: Communication, sum: TestSummary, logging) -> bool:
    start = time.time()
    logging.info("💾 Starting External Flash Testing 💾")
    if send_data(com, bytes(), MessageType.REQ_TEST_EXT_FLASH) is False:
        print_summary_error_and_exit(
            "Sending REQ_TEST_EXT_FLASH isn't successful!", ExitCodes.SENDING_FAILED, logging
        )
    ...
    if extflash_status != TestStatus.SUCCESS:
        print_summary_error_and_exit(
            "❌ External Flash is not OK ❌", ExitCodes.TEST_FAILED, logging
        )
    else:
...
def perform_rf_testing(com: Communication, sum: TestSummary, logging) -> bool:
    start = time.time()
    rx_rssi, tx_rssi = None, None
    logging.info("📶 Starting RF Testing 📶")
    if send_data(com, bytes(), MessageType.REQ_TEST_RF) is False:
        print_summary_error_and_exit(
            "Sending REQ_TEST_RF isn't successful!", ExitCodes.SENDING_FAILED, logging
        )
    ...
    if sum.status != TestStatus.SUCCESS:
        ...
        print_summary_error_and_exit(message, ExitCodes.TEST_FAILED, logging)
    else:
        message = "✅ RF is OK ✅" + rx_tx_message
        logging.info(message)

    return sum, rx_rssi, tx_rssi
...
def run_rf_external_flash_test(serial_port, test_router_address, network_address, network_channel, encryption_key, authentication_key, serial_number, RX_RSSI_THRESHOLD_DBM, TX_RSSI_THRESHOLD_DBM):
    try:
        global extflash_summary
        global rf_summary
        child_logger = create_child_logger(serial_port, serial_number)
        ...
        test_router_address=validate_node_address(test_router_address),
        network_address = validate_network_address(network_address),
        network_channel = validate_network_channel(network_channel),
        encryption_key = validate_key(encryption_key),
        authentication_key = validate_key(authentication_key),
        serial_number = validate_serial_number(serial_number)
        )

        child_logger.debug(args)

        com = Communication()
        serial_initialization(args.serial_port, com, child_logger)

        nic_serial_number = args.serial_number
        initialize_dut(args, com, RX_RSSI_THRESHOLD_DBM, TX_RSSI_THRESHOLD_DBM, child_logger)

        extflash_summary = perform_external_flash_testing(com, extflash_summary, child_logger)
        time.sleep(1) 
        rf_summary, rx_rssi, tx_rssi = perform_rf_testing(com, rf_summary, child_logger)
        # print_testing_summary(extflash_summary, rf_summary, script_launch_time)

        if rf_summary.status == 0:
            result_file_name = f"{result_folder}/{serial_number}_RF_Test_result_PASS.log"
        else:
            result_file_name = f"{result_folder}/{serial_number}_RF_Test_result_FAIL.log"

        for handler in child_logger.handlers:
            handler.close()
            child_logger.removeHandler(handler)
        os.replace(f"{result_folder}/{serial_number}.log", result_file_name)

        return (extflash_summary.status , rf_summary.status , rx_rssi, tx_rssi)

    # print(run_rf_external_flash_test('COM19', '3', "0xF0EE0F", '3', '33333333333333333333333333333333', '44444444444444444444444444444444', 'P25123101000006900400761148'))
    except Exception as e:
        print("Error in rf test: ", str(e))