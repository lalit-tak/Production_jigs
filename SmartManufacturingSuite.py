...
def insert_into_ems_text(data_row):
    if data_row[2] == "":
        ...
        with open(result_file_path, mode) as f:
            ...
            f.write(f"End Time :- {data_row[4]}\n")
            f.write(f"Overall Result :- {data_row[-3]}\n")
            f.write(f"Total Time :- {data_row[-2]}\n")
            f.write(f"FirmwareFlash :- {data_row[5]}\n")
            f.write(f"Live Version Status :- {data_row[6]}\n")
            f.write(f"Live Version Value :- {data_row[7]}\n")
            f.write(f"NIC Status Result :- {data_row[8]}\n")
            f.write(f"NIC Status Value :- {data_row[9]}\n")
            f.write(f"Memory EEPROM Result :- {data_row[10]}\n")
            f.write(f"Memory Flash Result :- {data_row[11]}\n")
            f.write(f"3P Current R (A) :- {data_row[12]}\n")
            f.write(f"3P Current Y (A) :- {data_row[13]}\n")
            f.write(f"3P Current B (A) :- {data_row[14]}\n")
            f.write(f"3P Current N (A) :- {data_row[15]}\n")
            f.write(f"3P Voltage R (V) :- {data_row[16]}\n")
            f.write(f"3P Voltage Y (V) :- {data_row[17]}\n")
            f.write(f"3P Voltage B (V) :- {data_row[18]}\n")
            f.write(f"MD Reset Button Result :- {data_row[19]}\n")
            f.write(f"MD Reset Button Value :- {data_row[20]}\n")
            f.write(f"Magnet Status Result :- {data_row[21]}\n")
            f.write(f"Magnet Status Value :- {data_row[22]}\n")
            f.write(f"Push Button 1 Result :- {data_row[23]}\n")
            f.write(f"Push Button 1 Value :- {data_row[24]}\n")
            f.write(f"Push Button 2 Result :- {data_row[25]}\n")
            f.write(f"Push Button 2 Value :- {data_row[26]}\n")
            f.write(f"RX/TX Status Result :- {data_row[27]}\n")
            f.write(f"RX/TX Status Value :- {data_row[28]}\n")
            f.write(f"eFuse Mode Result :- {data_row[29]}\n")
            f.write(f"eFuse Mode Value :- {data_row[30]}\n")
            f.write(f"Cover Open Result :- {data_row[31]}\n")
            f.write(f"Cover Open Value :- {data_row[32]}\n")
        ...
        with self.lock:
            ...
            if not os.path.exists(self.main_file):
                ...
                headers = [
                    ...
                    "MD Reset Button Result", "MD Reset Button Value",
...
class ManufacturingSuite:
    def __init__(self, root):
        self.root = root
        self.root.title("Smart Manufacturing Suite : 3P- [SMS] [Version 1.0.0.2, 02-FEB-26]")
    ...
    def create_header(self):
        ...
        # Add title
        title_label = tk.Label(header_frame, text="Smart Manufacturing Suite [WC]", 
                              font=("Arial", 18, "bold"), bg="#1976D2", fg="white")
        title_label.pack(side=tk.LEFT, padx=5, pady=15)
        
        # Add version info
        version_label = tk.Label(header_frame, text="v1.0.0.2 | 02-FEB-26", 
