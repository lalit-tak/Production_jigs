...
class ManufacturingSuite:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("Smart Manufacturing Suite - [SMS] [Version 1.0.0.2, 12-May-26]")
        ...
        # Voltage value
        self.min_supercap_charge_voltage = 0
        self.max_supercap_charge_voltage = 0
        self.min_ldo_voltage = 0
        self.max_ldo_voltage = 0
        self.min_supercap_discharge_voltage = 0
        self.max_supercap_discharge_voltage = 0
    ...
    def load_channel_data(self):
        ...
        try:
            ...
            self.min_supercap_charge_voltage = criteria.get('min_supercap_charge_voltage')
            self.max_supercap_charge_voltage = criteria.get('max_supercap_charge_voltage')
            self.min_ldo_voltage = criteria.get('min_ldo_voltage')
            self.max_ldo_voltage = criteria.get('max_ldo_voltage')
            self.min_supercap_discharge_voltage = criteria.get('min_supercap_discharge_voltage')
            self.max_supercap_discharge_voltage = criteria.get('max_supercap_discharge_voltage')
    ...
    def create_header(self):
        ...
        # Add version info
        version_label = tk.Label(header_frame, text="v1.0.0.2 | 12-May-26", 
    ...
    def FirmwareFlash(self, is_main_firmware=False):
        ...
        logging.info(["python", Flashfile, os.path.join(folder_path, Config_file)])
        try:
    ...
    def module_supply_on_off_temp(self, input_val):
        try:
            script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "scripts", "tests", "staticDO.py")
            cmd = [sys.executable, script_path, "--inputVal", input_val]
            process = subprocess.Popen(
                    cmd,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
    ...
    def module_supply_on_off(self, input_val, supply_type):
        ...
        try:
            ...
            if error != "":
                logging.error(f"Module supply {supply_type} error: {error}")
                for station_num in range(1, 7):
                    if (station_num in self.task_indicators and 
                        station_num not in self.failed_stations and 
                        self.station_numbers[station_num].get().strip()):
                ...
                # verification_results = self.extract_verification_results(output)
                logging.info(f"Module supply {supply_type} results: {output}")
                ...
                for station_num, serial_var in self.station_numbers.items():
                    ...
                    try:
                        serial_number = serial_var.get().strip()  
                        if (status_match) and (station_num not in self.failed_stations):
    ...
    def read_ldo_supercap_voltage(self):
        ...
        try:
            ldo_voltage = 0
            super_cap_voltage = 0
            script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "scripts", "tests", "InstantAI.py")
            cmd = [sys.executable, script_path, "--deviceDescription", "USB-4716,BID#"]
            process = subprocess.Popen(
                    cmd,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
            ...
            if error != "":
                logging.error(f"Read LDO and Supercap Voltage error: {error}")
                for station_num in range(1, 7):
                    if (station_num in self.task_indicators and 
                        station_num not in self.failed_stations and 
                        self.station_numbers[station_num].get().strip()):
                ...
                # verification_results = self.extract_verification_results(output)
                logging.info(f"Read LDO and Supercap Voltage results: {output}")

                # Update task status based on DO result - SKIP FAILED STATIONS
                for station_num, serial_var in self.station_numbers.items():
                    serial_number = serial_var.get().strip() 
                    ldo_voltage = data_dict[f"Channel {int((station_num + 4))} data"]
                    ...
                    try:
                        ...
                        self.result[str(station_num)]["Super Cap Charge Voltage"]  = {}
                        self.result[str(station_num)]["LDO Voltage"]  = {}
                        if (super_cap_voltage >= self.min_supercap_charge_voltage) and (super_cap_voltage <= self.max_supercap_charge_voltage) and (station_num not in self.failed_stations):
                            ...
                            self.result[str(station_num)]["Super Cap Charge Voltage"]['value'] = str(super_cap_voltage)
                            self.task_indicators[station_num]["Super Cap Charge Voltage"].config(text="✕", fg="#F44336")
                        if (ldo_voltage >= self.min_ldo_voltage) and (ldo_voltage <= self.max_ldo_voltage)  and (station_num not in self.failed_stations):
    ...
    def read_discharge_supercap_voltage(self):
        ...
        try:
            ...
            if error != "":
                logging.error(f"Read Super Cap Discharge Voltage error: {error}")
                for station_num in range(1, 7):
                    if (station_num in self.task_indicators and 
                        station_num not in self.failed_stations and 
                        self.station_numbers[station_num].get().strip()):
                ...
                # verification_results = self.extract_verification_results(output)
                logging.info(f"Read Super Cap Discharge Voltage results: {output}")
                ...
                for station_num, serial_var in self.station_numbers.items():
                    ...
                    try:
                        # if status_match:
                        if (super_cap_dischareg_voltage >= self.min_supercap_discharge_voltage) and (super_cap_dischareg_voltage <= self.max_supercap_discharge_voltage) and (station_num not in self.failed_stations):
    ...
    def check_main_firmware_version(self):
        ...
        def perform_firmware_check(com_port, serial_number):
            ...
            try:
                ...
                print(com_port, serial_number, firmware_config,'-----------------')
                ...
                print(serial_number, result, version, expected )
    ...
    def run_test(self):
        ...
        self.module_supply_on_off("01", "On")
        self.root.update()
        ...
        self.read_ldo_supercap_voltage()
        self.root.update()
        ...
        self.module_supply_on_off_temp("02")
        self.status_label.config(text=f"Module Reset Wait for 11 sec...")
        self.root.update()
        time.sleep(10)
        self.module_supply_on_off_temp("01")
        ...
        self.read_discharge_supercap_voltage()
        self.root.update()
