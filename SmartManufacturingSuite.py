    ...
    def initialize_excel_files(self):
        ...
        with self.lock:
            ...
            if not os.path.exists(self.main_file):
                ...
                headers = [
                    "Test Session", "Station", "PCBA Serial Number", "Test Start Time", "Test End Time", "FW Flash Result", "Live Version Result", "Live Version Value", "RTC Calibration Result", "RTC Calibration Value", "Digital Input Result", "Digital Input Value",
                    "Overall Status", 
                    "Total Test Time (s)", "Timestamp"
                ]
...
class ManufacturingSuite:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("Smart Manufacturing Suite : 3P- [SMS] [Version 1.0.0.5, 22-Apr-26]")
    ...
    def create_header(self):
        ...
        # Add version info
        version_label = tk.Label(header_frame, text="v1.0.0.5 | 22-Apr-26", 
    ...
    def execute_dip_test(self):
        # Show popup on MAIN thread (non-blocking)
        self.root.after(0, self.show_popup)

        # Run test in BACKGROUND thread
        # script_path = os.path.join('digital_input', "digital_input_test.py")
        test_thread = threading.Thread(
            target=self._execute_tests_in_parallel,
            args=(
                "Digital Input Test",
    ...
    def _run_all_tests_sequence(self):
        ...
        while True:
            ...
            for serial_num in list(self.station_test_results.keys()):
                print(self.station_test_results[serial_num],'<<<<<<<<<<')
