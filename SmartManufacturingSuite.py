...
import MainFirmwareFlash
import test
import firmware_version_integration
...
try:
    ...
    workbook = Workbook()
    sheet = workbook.active
    sheet.append(["Station Number", "NIC serial number", "Start Time", "End Time", "Module Supply On", "LDO Voltage", "LDO Voltage Status", "Supercap Charging Voltage", "Supercap Charging Voltage Status", "Test Firmware Test", "RX RSSI", "TX RSSI", "RF Status", "External Flash", "Main Firmware Test", "Firmware Version", "Expected Firmware Version", "Firmware Version Test Status", "Module Supply Off", "Supercap Discharging Voltage", "Supercap Discharging Voltage Status", "Overall Result", "Total Time"])
...
def insert_into_ems_text(data_row):
    if data_row[1] == "":
        ...
        result_file_name = f"{data_row[1]}_{data_row[-2].upper()}.txt"
        ...
        with open(result_file_path, mode) as f:
            ...
            f.write("=" * 50 + "\n")
            f.write(f"Station Number :- {data_row[0]}\n")
            f.write(f"Serial Number :- {data_row[1]}\n")
            f.write(f"Start Time :- {data_row[2]}\n")
            f.write(f"End Time :- {data_row[3]}\n")
            f.write(f"Overall Result :- {data_row[-2]}\n")
            f.write(f"Total Time :- {data_row[-1]}\n")
            f.write(f"Module Supply On :- {data_row[4]}\n")
            f.write(f"LDO Voltage :- {data_row[5]}\n")
            f.write(f"LDO Voltage Status :- {data_row[6]}\n")
            f.write(f"Supercap Charging Voltage :- {data_row[7]}\n")
            f.write(f"Supercap Charging Voltage Status :- {data_row[8]}\n")
            f.write(f"Test Firmware Test :- {data_row[9]}\n")
            f.write(f"RX RSSI :- {data_row[10]}\n")
            f.write(f"TX RSSI :- {data_row[11]}\n")
            f.write(f"RF Status :- {data_row[12]}\n")
            f.write(f"External Flash :- {data_row[13]}\n")
            f.write(f"Main Firmware Test :- {data_row[14]}\n")
            f.write(f"Firmware Version :- {data_row[15]}\n")
            f.write(f"Expected Firmware Version :- {data_row[16]}\n")
            f.write(f"Firmware Version Test Status :- {data_row[17]}\n")
            f.write(f"Module Supply Off :- {data_row[18]}\n")
            f.write(f"Supercap Discharging Voltage :- {data_row[19]}\n")
            f.write(f"Supercap Discharging Voltage Status :- {data_row[20]}\n")
            f.write("=" * 50 + "\n\n")
        return True
...
def insert_into_excel(data_dict):
    ...
    for station_num in list(data_dict.keys()):
        # end_time = data_dict[(station_num)].get('end_time', '')
        end_time = datetime.now()
        overall = "FAIL"
        firmware_version = "FAIL"
        start_time = data_dict[(station_num)].get('start_time', '')
        ...
        test_firmware = data_dict[(station_num)].get("Test Firmware Download", {})
        main_firmware = data_dict[(station_num)].get("Main Firmware Download", {})
        ...
        overall_result = data_dict[(station_num)].get('Overall_result', 'FAIL')
        firmware_version_check = data_dict[(station_num)].get('Check_Firmware_Version', 'FAIL')
        ...
        row_data = [
            ...
            data_dict[(station_num)].get('start_time', ''),
            end_time,
            ...
            test_firmware.get('status', 'failed'),
            ...
            main_firmware.get('status', 'failed'),
...
class ModernUI:
    ...
    def create_task_indicator(parent, text, initial_state="pending"):
        ...
        icon_label = tk.Label(frame, text=state_config["symbol"], fg=state_config["color"], 
                             bg=parent["bg"], font=("Arial", 8, "bold"))
...
class ManufacturingSuite:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("Smart Manufacturing Suite - [SMS] [Version 1.0.0.3, 15-May-26]")
        ...
        self.router = {}
        self.rssiThreshold = {}
        self.load_port_config(self.CONFIG_FILE_NAME_ENC)
        # self.port_config = self.scanner_ports
    ...
    def load_port_config(self, path):
        ...
        if os.path.exists(path):
            try:
                ...
                self.port_config[1] = config.get('SCANNER_COM1', '')
                self.port_config[2] = config.get('SCANNER_COM2', '')
                self.port_config[3] = config.get('SCANNER_COM3', '')
                self.port_config[4] = config.get('SCANNER_COM4', '')
                self.port_config[5] = config.get('SCANNER_COM5', '')
                self.port_config[6] = config.get('SCANNER_COM6', '')
                ...
                self.test_firmware = config.get("Test_Firmware", None)
                self.main_firmware = config.get("Main_Firmware", None)

                # Load Gang Programming Code File
                self.gang_pro_code_file = config.get("Gang_Programming_Code_File", None)

                # Load Voltages
                self.voltage["min_ldo_voltage"] = config.get("min_ldo_voltage", 0.0)
                self.voltage["max_ldo_voltage"] = config.get("max_ldo_voltage", 0.0)
                self.voltage["min_supercap_charge_voltage"] = config.get("min_supercap_charge_voltage", 0.0)
                self.voltage["max_supercap_charge_voltage"] = config.get("max_supercap_charge_voltage", 0.0)
                self.voltage["min_supercap_discharge_voltage"] = config.get("min_supercap_discharge_voltage", 0.0)
                self.voltage["max_supercap_discharge_voltage"] = config.get("max_supercap_discharge_voltage", 0.0)
                ...
                self.station3_map = config.get("Gang_Programming_Station_Map", {}).get("Station3", "#3")
                self.station4_map = config.get("Gang_Programming_Station_Map", {}).get("Station4", "#4")
                self.station5_map = config.get("Gang_Programming_Station_Map", {}).get("Station5", "#5")
                self.station6_map = config.get("Gang_Programming_Station_Map", {}).get("Station6", "#6")

                # Load Router Configuration
                self.router['enc_key'] = config.get("ENCRYPTION_KEY", "33333333333333333333333333333333")
                self.router['auth_key'] = config.get("AUTHENTICATION_KEY", "44444444444444444444444444444444")
                self.router['network_add'] = config.get("NETWORK_ADDRESS", "0xF0EE0F")
                self.router['router_add'] = config.get("TEST_ROUTER_ADDRESS", "3")
                self.router['network_channel'] = config.get("NETWORK_CHANNEL", "3")

                # Load RX/TX Threshold
                self.rssiThreshold['rx_threshold'] = config.get("RX_RSSI_THRESHOLD_DBM", -50)
                self.rssiThreshold['tx_threshold'] = config.get("TX_RSSI_THRESHOLD_DBM", -50)

                logging.info(f"Loaded scanner ports: {self.port_config}")
                logging.info(f"Loaded Dack Card ports: {self.dec_port_id}")
                logging.info(f"Loaded Firmware version: {self.firmware_version}")
                logging.info(f"Loaded Test Firmware File: {self.test_firmware}")
                ...
                logging.info(f"Loaded Router Configuration: {self.router}")
                logging.info(f"Loaded RSSI RX/TX Threshold: {self.rssiThreshold}")
                logging.info(f"Loaded Timeout Duration: {self.timeout_duration} seconds")
                logging.info(f"Loaded GangPro station map: {config.get("Gang_Programming_Station_Map", {})}")
    ...
    def auto_scan_all_serials(self):
        ...
        for station_num, result in scan_results.items():
            self.result[str(station_num)] = {}
            self.result[str(station_num)]['start_time'] = datetime.now()
            if result['status'] == 'success' and result['serial']:
                ...
                successful_scans += 1
                self.result[str(station_num)]['serial_number'] = result['serial']
                ...
                self.result[str(station_num)]['serial_number'] = " "
                ...
                # self.result[f"Station_{station_num}_SCAN_FAILED"] = []
                # self.result[f"Station_{station_num}_SCAN_FAILED"].append(station_num)
                # self.result[f"Station_{station_num}_SCAN_FAILED"] = ['PASS', '', '', 'Fail', 'N/A', 'N/A', 'Fail', 'Fail', 'PASS', '', 'None', '6.1.0.0']
                # sheet.append([
    ...
    def create_header(self):
        ...
        # Add version info
        version_label = tk.Label(header_frame, text="v1.0.0.3 | 15-May-26", 
    ...
    def create_main_content(self):
        ...
        for i in range(2):
            for j in range(3):
                ...
                station_frame.grid(row=i, column=j, padx=10, pady=0, sticky="nsew")
                ...
                ModernUI.create_rounded_rectangle(station_canvas, 2, 2, 438, 358, radius=10, 
                                                fill="white", outline="#E0E0E0", width=2)
                
                # Create inner frame for content
                inner_frame = tk.Frame(station_canvas, bg="white")
                station_canvas.create_window(200, 185, window=inner_frame, width=380, height=350)
                ...
                station_label.pack(side=tk.LEFT, padx=15, pady=3)
                ...
                entry = tk.Entry(serial_frame, textvariable=self.station_numbers[station_num], 
                               width=100, font=("Arial", 10), bd=2, relief=tk.GROOVE)
                ...
                for task in tasks:
                    ...
                    task_indicator.pack(fill=tk.X, pady=0)
    ...
    def create_status_bar(self):
        ...
        # DAC version display
        self.daq_version_label = tk.Label(status_frame, text=f"DAQ Card Channel: 16", 
                                            bg="#E0E0E0", fg="#333", font=("Arial", 10 , "bold"))
        self.daq_version_label.pack(side=tk.RIGHT, padx=10)
    ...
    def RF_test_perform(self, serial_port, test_router_address, network_address, network_channel, encryption_key, authentication_key, serial_number):
        try:
            logging.info(f"Starting RF test for Serial Number: {serial_number} on Port: {serial_port}")
            try:
                args_var = [] 
                kwargs_var = {}
                args_var.append(serial_port)
                args_var.append(str(test_router_address))
                args_var.append(str(network_address))
                args_var.append(str(network_channel))
                args_var.append(str(encryption_key))
                args_var.append(str(authentication_key))
                args_var.append(str(serial_number))
                args_var.append(self.rssiThreshold['rx_threshold'])
                args_var.append(self.rssiThreshold['tx_threshold'])
                logging.debug(f"RF Test command: {args_var}")

                my_test_function = test.run_rf_external_flash_test
                ...
                data_dict = stdout
                logging.info(f"Read RF, External Flash results: {stdout}")
                logging.error(f"Read RF, External Flash error: {stderr}")
            ...
            rf_ok = 1 if stdout[1] == 0 else 0
            flash_ok = 1 if stdout[1] == 0 else 0
            ...
            # # Save result to file
            # result_folder = os.path.join(base_dir, "Detailed Test Result")
    ...
    def run_rf_test(self):
        ...
        def perform_rf_test(com_port, serial_number):
            ...
            tx_rssi = ""
            try:
                result = self.RF_test_perform(
                    serial_port=com_port,
                    test_router_address=self.router['router_add'],
                    network_address=self.router['network_add'],
                    network_channel=self.router['network_channel'],
                    encryption_key=self.router['enc_key'],
                    authentication_key=self.router['auth_key'],
                    serial_number=serial_number
                )

                serial, rf_status, flash_status, rx_rssi, tx_rssi, action = result
                # pattern = r"RF Status:\s*(\w+),\s*External Flash:\s*(\w+)"
                
                # match = re.search(pattern, action)
                # if match:
                #     rf_status = match.group(1)
                #     external_flash = match.group(2)
                    # print("RF Status:", rf_status)
                    # print("External Flash:", external_flash)

                # Save to Excel (thread-safe because each thread writes one row)
                # sheet.append([
                #     serial, rf_status, rx_rssi, tx_rssi, rf_status, external_flash,
                #     datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                # ])
                station_num = 1
                for station, value in self.result.items():
                    if serial == value['serial_number']:
                        station_num = station
                        break

                self.result[str(station_num)]["rf_test_result"] = rf_status
                self.result[str(station_num)]["tx_rssi"] = tx_rssi
                self.result[str(station_num)]["rx_rssi"] = rx_rssi
                self.result[str(station_num)]["rf_status"] = rf_status
                self.result[str(station_num)]["flash_status"] = flash_status
                # self.result[serial_number].append(rf_status)
                # self.result[serial_number].append(rx_rssi)
                # self.result[serial_number].append(tx_rssi)
                # self.result[serial_number].append(rf_status)
                # self.result[serial_number].append(flash_status)
                # workbook.save(file_name)
    ...
    def FirmwareFlash(self, is_main_firmware=False):
        ...
        Flashfile = MainFirmwareFlash.run_firmware_flash if is_main_firmware else FirmwareFlash.run_firmware_flash
        args_var = [] 
        kwargs_var = {} 
        ...
        count = 0
        for station_num, serial_var in self.station_numbers.items():
            self.result[str(station_num)][task_name] = {}
        ...
        my_test_function = Flashfile
        if is_main_firmware:
            firmware_file = self.main_firmware
        else:
            firmware_file = self.test_firmware
        args_var.append(firmware_file)
        args_var.append(self.gang_pro_code_file)
        logging.info(f"Function for test and main firmware functions: {my_test_function}, parameters: {args_var}")
        try:
            station_num = 0
            stdout, stderr, return_code, timed_out = self.process_manager.run_function_with_timeout(
                my_test_function,
            ...
            if stderr:
                logging.error(f"Firmware flash error: {stderr}")
                messagebox.showerror("Error", f"Failed to execute firmware flash: {stderr}")
                ...
                for station, result in verification_results.items():
                    try:
                        ...
                        if station_num not in self.failed_stations:
                            if result == "OK":
                                self.result[str(station_num)][task_name]['status'] = result
    ...
    def module_supply_on_off_temp(self, input_val, supply_type):
        try:
            args_var = [] 
            kwargs_var = {}
            my_test_function = staticDO.run_staticDO
            args_var.append(input_val)
            ...
            stdout, stderr, return_code, timed_out = self.process_manager.run_function_with_timeout(
                ...
                self.timeout_duration,
                None,
                f'Module_Supply_{supply_type}',
                *args_var,
                **kwargs_var
            )

        except Exception as e:
            logging.error(f"Failed to Module Reset Wait for 7 sec: {e}")
            messagebox.showerror("Error", f"Failed to Module Reset Wait for 7 sec: {e}")

    def module_supply_on_off(self, input_val, supply_type):
        self.status_label.config(text=f"Running Module Supply {supply_type.lower()}...")
        for station_num, serial_var in self.station_numbers.items():
            # Skip failed stations
            if station_num in self.failed_stations:
                continue
            ...
            if serial_number:
                # Update task indicator to show firmware flash is in progress
                self.task_indicators[station_num][f"Module Supply {supply_type.title()}"].config(text="⏳", fg="#2196F3")
        ...
        try:
            ...
            if stderr != "":
                logging.error(f"Module supply {supply_type} error: {stderr}")
                for station_num in range(1, 7):
                    if (station_num in self.task_indicators and 
                        station_num not in self.failed_stations and 
                        self.station_numbers[station_num].get().strip()):
                        self.task_indicators[station_num][f"Module Supply {supply_type.title()}"].config(text="✕", fg="#F44336")
            else:
                # verification_results = self.extract_verification_results(output)
                logging.info(f"Module supply {supply_type} results: {stdout}")

                # Update task status based on DO result - SKIP FAILED STATIONS
                
                for station_num, serial_var in self.station_numbers.items():
                    self.result[str(station_num)][f'Module_Supply_{supply_type}'] = {}
                    try:
                        serial_number = serial_var.get().strip()  
                        if ("do output completed" in stdout.lower()) and (station_num not in self.failed_stations):
                            self.result[str(station_num)][f'Module_Supply_{supply_type}']['status'] = "PASS"
                            self.task_indicators[station_num][f"Module Supply {supply_type.title()}"].config(text="✓", fg="#4CAF50")
                        else:
                            self.result[str(station_num)][f'Module_Supply_{supply_type}']['status'] = "FAIL"
                            self.task_indicators[station_num][f"Module Supply {supply_type.title()}"].config(text="✕", fg="#F44336")
                    except (ValueError, KeyError) as e:
                        logging.error(f"Error updating station {station_num} status: {e}")

                        

        except Exception as e:
            logging.error(f"Failed to On Module Supply: {e}")
            print("Error: ",str(e))
            messagebox.showerror("Error", f"Failed to On Module Supply: {e}")
            for station_num, serial_var in self.station_numbers.items():
                serial_number = serial_var.get().strip()  
                if (station_num in self.task_indicators and 
                    station_num not in self.failed_stations and 
    ...
    def read_ldo_supercap_voltage(self, task_name):
        self.status_label.config(text=f"Running Read LDO and Supercap Voltage...")
        for station_num, serial_var in self.station_numbers.items():
            # Skip failed stations
            if station_num in self.failed_stations:
                continue
            ...
            if serial_number:
                # Update task indicator to show firmware flash is in progress
                self.task_indicators[station_num][f"LDO Voltage"].config(text="⏳", fg="#2196F3")
        ...
        try:
            ...
            data_dict = stdout
            logging.info(f"Read LDO and Supercap Voltage results: {stdout}")
            logging.error(f"Read LDO and Supercap Voltage error: {stderr}")
            if len(data_dict.keys()) >= 12:
                test_status = "PASS"
                
            else:
                test_status = "FAIL"

            if (stderr != "") or (test_status == 'FAIL'):
                for station_num in range(1, 7):
                    if (station_num in self.task_indicators and 
                        station_num not in self.failed_stations and 
                        self.station_numbers[station_num].get().strip()):
                        self.result[str(station_num)]["Super Cap Charge Voltage"]  = {}
                        self.result[str(station_num)]["LDO Voltage"]  = {}
                        self.task_indicators[station_num]["LDO Voltage"].config(text="✕", fg="#F44336")
                        self.task_indicators[station_num]["Super Cap Charge Voltage"].config(text="✕", fg="#F44336")
                        self.result[str(station_num)]["Super Cap Charge Voltage"]['status'] = "FAIL"
                        self.result[str(station_num)]["Super Cap Charge Voltage"]['value'] = str(super_cap_voltage)
                        self.result[str(station_num)]["LDO Voltage"]['status'] = "FAIL"
                        self.result[str(station_num)]["LDO Voltage"]['value'] = str(ldo_voltage)
            else:
                # verification_results = self.extract_verification_results(output)
                ...
                for station_num, serial_var in self.station_numbers.items():
                    serial_number = serial_var.get().strip() 
                    ldo_voltage = data_dict[f"Channel {int((station_num + 6))} data"]
                    super_cap_voltage = data_dict[f"Channel {int((station_num - 1))} data"]

                    try:
                        # if status_match:
                        self.result[str(station_num)]["Super Cap Charge Voltage"]  = {}
                        self.result[str(station_num)]["LDO Voltage"]  = {}
                        if (super_cap_voltage >= self.voltage["min_supercap_charge_voltage"]) and (super_cap_voltage <= self.voltage["max_supercap_charge_voltage"]) and (station_num not in self.failed_stations):
                            self.result[str(station_num)]["Super Cap Charge Voltage"]['status'] = "PASS"
                            self.result[str(station_num)]["Super Cap Charge Voltage"]['value'] = str(super_cap_voltage)
                            self.task_indicators[station_num]["Super Cap Charge Voltage"].config(text="✓", fg="#4CAF50")
                        else:
                            self.result[str(station_num)]["Super Cap Charge Voltage"]['status'] = "FAIL"
                            self.result[str(station_num)]["Super Cap Charge Voltage"]['value'] = str(super_cap_voltage)
                            self.task_indicators[station_num]["Super Cap Charge Voltage"].config(text="✕", fg="#F44336")
                        if (ldo_voltage >= self.voltage["min_ldo_voltage"]) and (ldo_voltage <= self.voltage["max_ldo_voltage"])  and (station_num not in self.failed_stations):
                            self.result[str(station_num)]["LDO Voltage"]['status'] = "PASS"
                            self.result[str(station_num)]["LDO Voltage"]['value'] = str(ldo_voltage)
                            self.task_indicators[station_num]["LDO Voltage"].config(text="✓", fg="#4CAF50")
                        else:
                            self.result[str(station_num)]["LDO Voltage"]['status'] = "FAIL"
                            self.result[str(station_num)]["LDO Voltage"]['value'] = str(ldo_voltage)
                            self.task_indicators[station_num]["LDO Voltage"].config(text="✕", fg="#F44336")
            ...
            for station_num, serial_var in self.station_numbers.items():
                if (station_num in self.task_indicators and 
                    ...
                    self.station_numbers[station_num].get().strip()):
                    self.result[str(station_num)]["Super Cap Charge Voltage"]['status'] = "FAIL"
                    self.result[str(station_num)]["Super Cap Charge Voltage"]['value'] = " "
                    self.result[str(station_num)]["LDO Voltage"]['status'] = "FAIL"
                    self.result[str(station_num)]["LDO Voltage"]['value'] = " "
                    self.task_indicators[station_num]["LDO Voltage"].config(text="✕", fg="#F44336")
                    self.task_indicators[station_num]["Super Cap Charge Voltage"].config(text="✕", fg="#F44336")

    def read_discharge_supercap_voltage(self, task_name):
        self.status_label.config(text=f"Running Read Super Cap Discharge Voltage ...")
        for station_num, serial_var in self.station_numbers.items():
            # Skip failed stations
            if station_num in self.failed_stations:
                continue
            ...
            if serial_number:
                # Update task indicator to show firmware flash is in progress
                self.task_indicators[station_num][f"Super Cap Discharge Voltage"].config(text="⏳", fg="#2196F3")
        
        self.root.update()
        try:
            super_cap_dischareg_voltage = 0
            args_var = [] 
            kwargs_var = {}
            ldo_voltage = 0
            super_cap_voltage = 0
            ...
            if (stderr != "") or (test_status == "FAIL"):
                for station_num in range(1, 7):
                    if (station_num in self.task_indicators and 
                        station_num not in self.failed_stations and 
                        self.station_numbers[station_num].get().strip()):
                        self.result[str(station_num)]["Super Cap Discharge Voltage"]  = {}
                        self.task_indicators[station_num]["Super Cap Discharge Voltage"].config(text="✕", fg="#F44336")
                        self.result[str(station_num)]["Super Cap Discharge Voltage"]['status'] = "FAIL"
                        self.result[str(station_num)]["Super Cap Discharge Voltage"]['value'] = str(super_cap_dischareg_voltage)
            else:
                ...
                for station_num, serial_var in self.station_numbers.items():
                    ...
                    super_cap_dischareg_voltage = data_dict[f"Channel {int((station_num - 1))} data"]
                    # super_cap_dischareg_voltage = 0.8
                    try:
                        # if status_match:
                        if (super_cap_dischareg_voltage >= self.voltage["min_supercap_discharge_voltage"]) and (super_cap_dischareg_voltage <= self.voltage["max_supercap_discharge_voltage"]) and (station_num not in self.failed_stations):
                            self.result[str(station_num)]["Super Cap Discharge Voltage"]['status'] = "PASS"
                            self.result[str(station_num)]["Super Cap Discharge Voltage"]['value'] = str(super_cap_dischareg_voltage)
                            self.task_indicators[station_num]["Super Cap Discharge Voltage"].config(text="✓", fg="#4CAF50")
                        else:
                            self.result[str(station_num)]["Super Cap Discharge Voltage"]['status'] = "FAIL"
                            self.result[str(station_num)]["Super Cap Discharge Voltage"]['value'] = str(super_cap_dischareg_voltage)
                            self.task_indicators[station_num]["Super Cap Discharge Voltage"].config(text="✕", fg="#F44336")
            ...
            logging.error(f"Failed to Read Super Cap Discharge Voltage Discharge: {e}")
            messagebox.showerror("Error", f"Failed to Read Super Cap Discharge Voltage Discharge: {e}")
            for station_num, serial_var in self.station_numbers.items():
                if (station_num in self.task_indicators and 
                    station_num not in self.failed_stations and 
                    self.station_numbers[station_num].get().strip()):
                    self.result[str(station_num)]["Super Cap Discharge Voltage"]['status'] = "FAIL"
                    self.result[str(station_num)]["Super Cap Discharge Voltage"]['value'] = " "
                    self.task_indicators[station_num]["Super Cap Discharge Voltage"].config(text="✕", fg="#F44336")
            # self.root.after(500, self.update_status_after_tests)
        finally:
                self.root.after(500, self.update_status_after_tests)
    ...
    def check_main_firmware_version(self):
        ...
        firmware_config = self.firmware_version
        ...
        def perform_firmware_check(com_port, serial_number):
            ...
            try:
                ...
                ldo_voltage = 0
                super_cap_voltage = 0
                my_test_function = firmware_version_integration.check_firmware_version
                args_var.append(com_port)
                args_var.append(serial_number)
                args_var.append(firmware_config)
                stdout, stderr, return_code, timed_out = self.process_manager.run_function_with_timeout(
                    my_test_function,
                    self.timeout_duration,
                    station_num,
            ...
            def update_ui_after_firmware_completion():
                ...
                for station_num, serial_var in self.station_numbers.items():
                    ...
                    if serial_number in results:
                        ...
                        self.result[str(station_num)]['Check_Firmware_Version'] = result
                        self.result[str(station_num)]['Firmware_Version'] = str(version)
                        self.result[str(station_num)]['Expected_Firmware_Version'] = str(expected)
    ...
    def run_test(self):
        ...
        # Step 2: Module Supply On
        self.module_supply_on_off("06", "On")
        self.root.update()
        self.status_label.config(text=f"Wait for 45 seconds")
        self.root.update()
        time.sleep(45)

        # Step 3: Read LDO and Supercap Voltage
        self.read_ldo_supercap_voltage("READ_CHARGING_LDO_SUPERCAP_VOLTAGE")
        self.root.update()


        # Step 4: Test Firmware Download
        self.FirmwareFlash(is_main_firmware=False)
        self.root.update()  # Force UI update

        self.module_supply_on_off_temp("05", "AFTER_TEST_FIRMWARE_CLOSE")
        self.status_label.config(text=f"Module Reset Wait for 11 sec...")
        self.root.update()
        time.sleep(10)
        self.module_supply_on_off_temp("06", "AFTER_TEST_FIRMWARE_ON")
        ...
        # Step 6: Main Firmware Download
        self.FirmwareFlash(is_main_firmware=True)
        self.root.update()  # Force UI update

        self.module_supply_on_off_temp("05", "AFTER_MAIN_FIRMWARE_CLOSE")
        self.status_label.config(text=f"Module Reset Wait for 10 sec...")
        self.root.update()
        time.sleep(10)
        self.module_supply_on_off_temp("06", "AFTER_MAIN_FIRMWARE_ON")

        time.sleep(5)
        
        # Step 7: Verify Main Firmware Version
        self.check_main_firmware_version()
        self.status_label.config(text=f"Wait for check firmware version wait 5 second")
        self.root.update()
        time.sleep(5)
        # time.sleep(0.5)  # Allow user to see changes

        # Step 8: Module Supply Off
        self.module_supply_on_off("05", "Off")
        self.status_label.config(text=f"Wait for 10 seconds")
        self.root.update()
        time.sleep(10)  # Wait to discharge super cap

        # Step 9: Read Discharge Super Cap Disharge Voltage
        self.read_discharge_supercap_voltage("READ_DISCHARGING_LDO_SUPERCAP_VOLTAGE")
        self.root.update()
    ...
    def update_status_after_tests(self):
        ...
        for station_num, serial_var in self.station_numbers.items():
            ...
            if serial_number:
                ...
                all_tests_passed = self.check_all_tests_status(station_num)
                # all_tests_passed = self.check_overall_result()
                # print(all_tests_passed,'-------------------------------')
                self.result[str(station_num)]["Overall_result"] = all_tests_passed
                self.result[str(station_num)]["end_time"] = datetime.now()
