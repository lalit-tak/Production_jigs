...
class ManufacturingSuite:
    def __init__(self, root):
        ...
        self.connectivity_alarm_stations = set()  # set of (station_num, serial_number, voltage)
    ...
    def create_status_bar(self):
        ...
        self.deck_card_count = tk.Label(status_frame, text=f"Deck Card Count: 1 - 16 CH", 
                                            bg="#E0E0E0", fg="#333", font=("Arial", 10 , "bold"))
                ...
                for com_port, serial_number, station_num in self.active_ports_serials:
                    if serial_number.strip():
                        ...
                        if task_name == "Boost Output":
                            super_cap_voltage = float(data_dict["Channel " + str((station_num - 1) + 0 ) + " data"])
                            Boost_voltage = float(data_dict["Channel " + str((station_num - 1) + 4 ) + " data"])
                            if (super_cap_voltage >= self.voltage["min_super_cap_voltage"]) and (super_cap_voltage <= self.voltage["max_super_cap_voltage"]):
                                self.station_test_results[serial_number]["Super Cap Voltage"] = {"status": "PASS", "value": data_dict["Channel " + str((station_num - 1)) + " data"]}
                                self.root.after(0, lambda s=station_num, t="Super Cap Voltage", stat="PASS": self._update_task_ui(s, "Super Cap Voltage", "PASS"))
                            else:
                                self.station_test_results[serial_number]["Super Cap Voltage"] = {"status": "FAIL", "value": data_dict["Channel " + str((station_num - 1)) + " data"]}
                            ...
                            if (float(Boost_voltage) >= self.voltage["min_Boost_voltage"]) and (float(Boost_voltage) <= self.voltage["max_Boost_voltage"]):
                                self.station_test_results[serial_number]["Boost Output"] = {"status": "PASS", "value": data_dict["Channel " + str((station_num - 1) + 4 ) + " data"]}
                                self.root.after(0, lambda s=station_num, t="Boost Output", stat="PASS": self._update_task_ui(s, "Boost Output", "PASS"))
                            else:
                                self.station_test_results[serial_number]["Boost Output"] = {"status": "FAIL", "value": data_dict["Channel " + str((station_num - 1) + 4 ) + " data"]}
                            ...
                            Dis_super_cap_voltage = float(data_dict["Channel " + str((station_num - 1) + 0 ) + " data"])
                            Discharge_voltage = float(data_dict["Channel " + str((station_num - 1) + 4 ) + " data"])
                            ...
                            if (Dis_super_cap_voltage >= self.voltage["min_Dis_super_cap_voltage"]) and (Dis_super_cap_voltage <= self.voltage["max_Dis_super_cap_voltage"]):
                                self.station_test_results[serial_number]["Discharge Super Cap Voltage"] = {"status": "PASS", "value": data_dict["Channel " + str((station_num - 1) + 0) + " data"]}
                                ...
                                self.station_test_results[serial_number]["Discharge Super Cap Voltage"] = {"status": "FAIL", "value": data_dict["Channel " + str((station_num - 1) + 0) + " data"]}
                            ...
                            if (float(Discharge_voltage) >= self.voltage["min_Discharge_voltage"]) and (float(Discharge_voltage) <= self.voltage["max_Discharge_voltage"]):
                                self.station_test_results[serial_number]["DISCHARGE OUTPUT"] = {"status": "PASS", "value": data_dict["Channel " + str((station_num - 1) + 4) + " data"]}
                                self.root.after(0, lambda s=station_num, t="DISCHARGE OUTPUT", stat="PASS": self._update_task_ui(s, "DISCHARGE OUTPUT", "PASS"))
                            else:
                                self.station_test_results[serial_number]["DISCHARGE OUTPUT"] = {"status": "FAIL", "value": data_dict["Channel " + str((station_num - 1) + 4) + " data"]}
                                self.root.after(0, lambda s=station_num, t="DISCHARGE OUTPUT", stat="FAIL": self._update_task_ui(s, "DISCHARGE OUTPUT", "FAIL"))
                                # Connectivity alarm: voltage below minimum indicates supply pin not seated
                                if float(Discharge_voltage) < self.voltage["min_Discharge_voltage"]:
                                    self.root.after(0, lambda sn=station_num, sr=serial_number, dv=float(Discharge_voltage):
                                                    self._show_discharge_connectivity_alarm(sn, sr, dv))
    ...
    def _show_discharge_connectivity_alarm(self, station_num, serial_number, discharge_voltage):
        """Show connectivity warning when discharge voltage is below minimum — called on main thread"""
        messagebox.showwarning(
            f"⚠ Discharge Connectivity Issue — Station {station_num}",
            f"Station {station_num}  |  Serial: {serial_number}\n\n"
            f"Discharge voltage read : {discharge_voltage:.3f} V\n"
            f"Expected range         : {self.voltage['min_Discharge_voltage']} – {self.voltage['max_Discharge_voltage']} V\n\n"
            f"Module supply pin may not be properly seated.\n"
            f"Please reseat the module and re-run the test."
        )
        logging.warning(
            f"Station {station_num} ({serial_number}): Connectivity alarm — "
            f"Discharge voltage {discharge_voltage:.3f} V below min {self.voltage['min_Discharge_voltage']} V"
        )
