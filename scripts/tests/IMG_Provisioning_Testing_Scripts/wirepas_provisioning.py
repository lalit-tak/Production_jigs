...
class NodeProvisioningDataIds(IntEnum):
    ...
    WIREPAS_BAND_LOCK = 165                  # PROV_DATA_ID_BAND_LOCK
    WIREPAS_SIM_SELECTION = 167              # PROV_DATA_ID_SIM_SELECTION
...
class NicInfoDataIds(IntEnum):
    ...
    GW_METER_LIB_VER = 232        # NIC_INFO_GW_METER_LIB_VER
...
# These must match the SIM_SELECTION_STRINGS array in configuration.c
SIM_SELECTION_OPTIONS = [
    'not_set',
    'single_sim1', 
    'single_sim2',
    'dual_prefer_sim1',
...
class WirepasProvisioning:
    ...
    def __init__(
        ...
        self.sim2_apn = sim2_apn
        self.sim2_username = sim2_username
        self.sim2_password = sim2_password
        self.sim2_pdp_type = sim2_pdp_type
        self.sim2_pin = sim2_pin
        self.sim2_puk = sim2_puk
    ...
    def _generate_provisioning_dict(self) -> dict:
        prov_dict = {
            ...
            NodeProvisioningDataIds.WIREPAS_SIM2_APN.value: self.sim2_apn,
            NodeProvisioningDataIds.WIREPAS_SIM2_USERNAME.value: self.sim2_username,
            NodeProvisioningDataIds.WIREPAS_SIM2_PASSWORD.value: self.sim2_password,
            NodeProvisioningDataIds.WIREPAS_SIM2_PDP_TYPE.value: self.sim2_pdp_type,
            NodeProvisioningDataIds.WIREPAS_SIM2_PIN.value: self.sim2_pin,
            NodeProvisioningDataIds.WIREPAS_SIM2_PUK.value: self.sim2_puk,
            ...
            NodeProvisioningDataIds.WIREPAS_BAND_LOCK.value: self.band_lock,
            NodeProvisioningDataIds.WIREPAS_SIM_SELECTION.value: self.sim_selection,
        }
...
def parse_locked_band_bits(band_bits_hex: str) -> list:
    # Convert the hexadecimal string to an integer
    band_bits = int(band_bits_hex, 16)
    
    # Initialize an empty list to store the locked bands
    locked_bands = []
    
    # Iterate over each bit position (1 to 256)
    for band_number in range(1, 257):
        # Check if the bit at the current position is set
        if band_bits & (1 << (band_number - 1)):
            locked_bands.append(f"b{band_number}")
    
    if not locked_bands:
        locked_bands.append(None)
...
def decode_prov_dict(prov_read_data: bytes) -> bool:
    try:
        ...
        if NodeProvisioningDataIds.WIREPAS_BAND_LOCK.name in nic_params:
            nic_params[NodeProvisioningDataIds.WIREPAS_BAND_LOCK.name] = parse_locked_band_bits(
                nic_params[NodeProvisioningDataIds.WIREPAS_BAND_LOCK.name]
            )
        ...
        if NodeProvisioningDataIds.WIREPAS_SIM2_PASSWORD.name in nic_params:
            nic_params[NodeProvisioningDataIds.WIREPAS_SIM2_PASSWORD.name] = update_password_value(
                nic_params[NodeProvisioningDataIds.WIREPAS_SIM2_PASSWORD.name]
            )
