...
def decode_NIC_status(packet):
    try:
        if len(packet) >= 15:
            status = packet[12]
            print(f"🔌 Receive Port ID : {packet[9]}")
            print(f"🕒 Cmd Time        : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            print(f"✅ NIC Status      : {status}")
            return status
        else:
            print("❌ Packet too short to decode NIC status.")
            return None
    except Exception as e:
        print(f"❌ Failed to decode NIC status: {e}")
        return None

def get_NIC_status(serial_port):
    tx_data = bytes([
        0xAA, 0x1, 0x4E, 0x47, 0xC9,
        0xFF, 0xFF, 0xFF, 0xFF,
        0x4, 0x4, 0x2, 0xF2, 0x36,
        0x57, 0xFA
    ])
    ...
    try:
        ...
        if com.send_data(tx_data, fixed_packet_len=15): # Expecting 15 bytes response
            ...
            if response:
                return decode_NIC_status(response)
    finally:
        com.close()
...
if __name__ == "__main__":
    ...
    print("\n--- Checking NIC Status ---")
    status = get_NIC_status(args.serial_port)
    if status is not None:
        print(f"NIC Status: {'NIC is not present' if status else 'NIC is present'} (value={status})")
    else:
        print("❌ Failed to retrieve NIC status.")
